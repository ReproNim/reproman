...
def test_retrace_to_output_file(reprozip_spec2):
    with make_tempfile() as outfile:
        args = ["retrace", "--spec", reprozip_spec2, "--output-file", outfile]
        main(args)
    ...
    for itracer, protocol in enumerate(protocols):
        ...
        class FakeTracer(object):
            ...
            def __init__(self, session):
                assert session
                assert self._protocol, "No more protocols to go through, but were were asked to"
...
def _check_loop_protocol(protocols, files, tenvs, tfiles):
    ...
    dists, unknown_files = identify_distributions(files, session, tracer_classes=tracer_classes)
...
def test_retrace_loop_over_tracers():
    _check_loop_protocol(
        [[[("Env1", {"thefile"})]]],  # Tracers  # Tracer passes  # what to yield
        files=["thefile"],
        tenvs=["Env1"],
        tfiles={"thefile"},
    )
    ...
    _check_loop_protocol(
        [  # Tracers
            [  # Tracer passes
                [("Env1", {"thefile"})],  # what to yield
            ],
            [  # Tracer passes
                [("Env2", set())],  # what to yield
            ],
        ],
        files=["thefile"],
        tenvs=["Env1", "Env2"],
