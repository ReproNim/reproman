...
def test_diff_git():
    with swallow_outputs() as outputs:
        ...
        assert_in(
            "> branch None, commit 9d199f7fa7e6f691719e0860c5cf81193e815ad5 (/path/2/to/different/git/commit)",
            outputs.out,
        )
...
def test_diff_satisfies():
    with swallow_outputs() as outputs:
        args = ["diff", "--satisfies", diff_satisfies_1_yaml, diff_satisfies_2_yaml]
        rv = main(args)
        assert_equal(rv, 3)
        assert_in("Files:", outputs.out)
        assert_in("> /etc/c", outputs.out)
