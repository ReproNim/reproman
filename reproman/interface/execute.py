...
class TracedCommand(CommandAdapter):
    ...
    def __init__(self, resource, command, cmd_args, remote_dir=None, local_dir=None):
        ...
        self.exec_id = "{}-{}".format(time.strftime("%Y%m%d%H%M%S"), str(uuid.uuid4())[:2])

        # Local session variables
        local_cache_dir = local_dir or op.expanduser("~/.cache/reproman")
        self.local_tracer_dir = op.join(local_cache_dir, "tracers", self.tracer_md5sum)
        self.local_trace_dir = op.join(local_cache_dir, "traces", self.exec_id)
        self.local_tracer_gz = op.join(self.local_tracer_dir, "reproman_trace.gz")
        self.local_docker_shim = op.join(op.dirname(op.realpath(__file__)), "docker.shim")
    ...
    def _prepare_local(self):
        if not op.exists(self.local_tracer_gz):
            ...
            resp = requests.get(
                "https://github.com/ReproNim/reprozip/blob"
            ...
            with open(self.local_tracer_gz, "wb") as stream:
                ...
                if self.tracer_md5sum != downloaded_md5sum:
                    raise RuntimeError(
                        f"md5sum for downloaded tracer ({downloaded_md5sum}) "
    ...
    def _prepare_remote(self):
        ...
        remote_tracer_dir = op.join(remote_reproman_dir, "tracers", self.tracer_md5sum)
    ...
    def execute(self):
        ...
        remote_extra_trace_path = op.join(self.remote_trace_dir, self.extra_trace_file)
        cmd_prefix = [
            self.remote_tracer,
            "--logfile",
            op.join(self.remote_trace_dir, "tracer.log"),
            "--dbfile",
            op.join(self.remote_trace_dir, "trace.sqlite3"),
            "--",
        ]
...
CMD_CLASSES = {"plain": PlainCommand, "internal": InternalCommand, "trace": TracedCommand}
...
class Execute(Interface):
    ...
    @staticmethod
    def __call__(command, args, resref=None, resref_type="auto", internal=False, trace=False):
        ...
        lgr.info("Executed the %s command in the environment %s", command, env_resource.name)
        ...
        if error:
            lgr.error("Command %s failed to run in %s: %s", command, env_resource.name, error.msg)
