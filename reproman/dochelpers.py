...
def _split_out_parameters(initdoc):
    ...
    return (
        handle_docstring(result[0], polite=False).strip("\n"),
...
def _parse_parameters(paramdoc):
    ...
    lgr.debug('parseParameters: Given "%s", we split into %s' % (paramdoc, result))
    return result
...
def get_docstring_split(f):
    ...
    initdoc, params, suffix = _split_out_parameters(f.__doc__)
...
def borrowkwargs(cls=None, methodname=None, exclude=None):
    ...
    def _borrowkwargs(method):
        ...
        aplist += [i for i in oplist if not i[0] in skip_params.union(known_params)]
...
def exc_str(exc=None, limit=None):
    ...
    if limit is None:
        ...
        limit = int(os.environ.get("REPROMAN_EXC_STR_TBLIMIT", "1"))
    try:
