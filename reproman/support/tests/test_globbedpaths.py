...
def test_globbedpaths_get_sub_patterns():
    ...
    for pat, expected in [
        ...
        (op.join("s", "ss*", "path*"), [op.join("s", "ss*") + op.sep]),
        (op.join("s", "ss*" + op.sep), []),
        (op.join("s*", "ss", "path*"), [op.join("s*", "ss") + op.sep, "s*" + op.sep]),
        (
            op.join("s?", "ss", "sss*", "path*"),
            [op.join("s?", "ss", "sss*") + op.sep, op.join("s?", "ss") + op.sep, "s?" + op.sep],
        ),
    ]:
...
def test_globbedpaths(path=None):
    ...
    for patterns, expected in [
        ...
        (["*.txt", "*.dat"], {"1.txt", "2.dat", "bβ.dat", "3.txt"}),
        ([dotdir + "*.txt", "*.dat"], {dotdir + "1.txt", "2.dat", "bβ.dat", dotdir + "3.txt"}),
        (["subdir/*.txt"], {"subdir/1.txt", "subdir/2.txt"}),
        ([dotdir + "subdir/*.txt"], {dotdir + p for p in ["subdir/1.txt", "subdir/2.txt"]}),
        (["*.txt"], {"1.txt", "3.txt"}),
    ]:
    ...
    for patterns, expected in [
        ...
        ([dotdir + pardir + "*.txt"], {dotdir + pardir + p for p in ["1.txt", "3.txt"]}),
        (["subdir/"], {"subdir/"}),
    ]:
        gp = GlobbedPaths(patterns, pwd=subdir_path)
        eq_(set(gp.expand()), expected)
        eq_(set(gp.expand(full=True)), {op.join(subdir_path, p) for p in expected})

    # Full patterns still get returned as relative to pwd.
    gp = GlobbedPaths([op.join(path, "*.dat")], pwd=path)
    eq_(gp.expand(), ["2.dat", "bβ.dat"])

    # "." gets special treatment.
    gp = GlobbedPaths([".", "*.dat"], pwd=path)
    eq_(set(gp.expand()), {"2.dat", "bβ.dat", "."})
    eq_(gp.expand(dot=False), ["2.dat", "bβ.dat"])
    ...
    for expand, expected in [(True, ["2.dat", "bβ.dat"]), (False, ["*.dat"])]:
