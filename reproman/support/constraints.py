...
class EnsureStr(Constraint):
    ...
    def __call__(self, value):
        ...
        if len(value) < self._min_len:
            raise ValueError("%r is shorter than of minimal length %d" % (value, self._min_len))
        return value
    ...
    def __call__(self, value):
        ...
        for c in self.constraints:
            try:
                ...
                e_list.append(e)
        raise ValueError("all alternative constraints (%s) violated while testing value %r" % (self.constraints, value))
