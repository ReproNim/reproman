...
class SSH(Resource):
    ...
    host = attrib(default=attr.NOTHING, doc="DNS or IP address of server or ssh_config nickname")
    port = attrib(doc="Port to connect to on remote host")
    key_filename = attrib(doc="Path to SSH private key file")
    ...
    def connect(self, password=None):
        ...
        try:
            ...
            password = getpass.getpass(prompt="Password for {}: ".format(self.name))
            self._connection = Connection(
                self.host, user=self.user, port=self.port, connect_kwargs={"password": password}
            )
            self._connection_open()
    ...
    def get_session(self, pty=False, shared=None):
        ...
        return (PTYSSHSession if pty else SSHSession)(connection=self._connection)
...
class SSHSession(POSIXSession):
    ...
    def _execute_command(self, command, env=None, cwd=None, with_shell=False, handle_permission_denied=True):
        ...
        if result.return_code not in [0, None]:
            ...
            raise CommandError(str(command), msg, result.return_code, result.stdout, result.stderr)
        else:
            lgr.log(8, "Finished running %r with status %s", command, result.return_code)
