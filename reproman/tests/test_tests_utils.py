...
def test_with_tempfile_default_prefix(d1=None):
    ...
    full = short + "test_with_tempfile_default_prefix"
    if on_windows:
...
@pytest.mark.parametrize("func", [os.chdir, chpwd], ids=["chdir", "chpwd"])
def test_assert_Xwd_unchanged(func):
    ...
    eq_(orig_pwd, getpwd(), "assert_cwd_unchanged didn't return us back to pwd %s" % orig_pwd)
...
def test_assert_cwd_unchanged_not_masking_exceptions():
    ...
    with swallow_logs(new_level=logging.WARN) as cml:
        ...
        eq_(orig_cwd, os.getcwd(), "assert_cwd_unchanged didn't return us back to %s" % orig_cwd)
...
def test_serve_path_via_http(test_fpath, tmpdir):  # pragma: no cover
    ...
    if test_fpath_encoded.decode(filesysencoding) != test_fpath:
        pytest.skip("Can't convert back/forth using %s encoding" % filesysencoding)
...
def test_assert_is_subset_recur():
    ...
    assert_raises(AssertionError, assert_is_subset_recur, [3, [2, 1]], [3, [2]], [list])
    assert_is_subset_recur([3, [2]], [3, [2]], [dict])
    assert_raises(AssertionError, assert_is_subset_recur, [3, [2]], [3, [2, 1]], [dict])
