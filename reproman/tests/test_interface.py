...
class Demo(Interface):
    ...
    _params_ = dict(
        demoposarg=Parameter(doc="demoposdoc", constraints=cnstr.EnsureInt(), nargs=2),
        demooptposarg1=Parameter(
            args=("demooptposarg1",), doc="demooptposdoc1", constraints=cnstr.EnsureInt(), nargs="?"
        ),
        demooptposarg2=Parameter(
            args=("demooptposarg2",), doc="demooptposdoc2", constraints=cnstr.EnsureInt(), nargs="?"
        ),
        demoarg=Parameter(doc="demodoc", constraints=cnstr.EnsureInt()),
    )
...
def test_param():
    ...
    assert_in("Detected unknown argument(s) for the Parameter: unknown_arg", str(cmr.value))
...
def test_name_generation():
    ...
    assert_equal(get_api_name(("some.module", "SomeClass", "cmdline-override")), "module")
    assert_equal(
