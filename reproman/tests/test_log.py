...
def test_logging_to_a_file(dst=None):
    ...
    line = lines[0]
    ok_(msg in line)
    ok_(not "\033[" in line, msg="There should be no color formatting in log files. Got: %s" % line)
    ...
    ok_(re.match(r"\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2},\d{3} \[ERROR\] %s" % msg, line))
...
def test_mutliple_targets(dst1=None, dst2=None):
    ok_(not exists(dst1))
    ok_(not exists(dst2))
    lgr = LoggerHelper("repromantest").get_initialized_logger(logtarget="%s,%s" % (dst1, dst2))
    ok_(exists(dst1))
    ok_(exists(dst2))
