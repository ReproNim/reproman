...
class HelpAction(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        if is_interactive() and option_string == '--help':
            ...
            try:
                import subprocess
                # get the niceman manpage to use
                ...
                # don't show manpage if man_version not equal to current niceman_version
...
def run_via_pbs(args, pbs):
    ...
    f = NamedTemporaryFile('w', prefix='niceman-%s-' % pbs, suffix='.submit', delete=False)
    try:
        pwd = getpwd()
...
def get_repo_instance(path=curdir, class_=None):
    """Returns an instance of appropriate niceman repository for path.
        ...
        if exists(opj(dir_, '.git')):
            ...
            if class_ is None:
                ...
                try:
                    ...
                    raise RuntimeError("No niceman repository found in %s" %
                                       abspath_)
            else:
                try:
    ...
    if class_ is not None:
        ...
        raise RuntimeError("No niceman repository found in %s" % abspath_)
