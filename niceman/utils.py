...
def abbreviate(paths):
    """Generate a short but unqiue name for a set of `paths`.

    If the basename is unique, use that.  Otherwise, append parent
    directories until the conflicting paths are unique.

    Parameters
    ----------
    paths : iterable of strings
        Paths to uniquely abbreviate.

    Returns
    -------
    dict that maps each element in `paths` to its unique name
    """
    # Make sure they are unique and then ordered.
    paths = list(set(paths))

    if any(p in [os.path.pathsep, os.path.sep, ""] for p in paths):
        raise ValueError("paths contain an empty path")

    basenames = list(map(os.path.basename, paths))
    candidates = dict(zip(paths, basenames))

    # Easy case: All basenames are unique.
    if len(paths) == len(set(basenames)):
        return candidates

    # The basenames of at least two paths conflict.
    def extend(shorts, unique):
        if not shorts:
            return unique

        counts = collections.Counter(shorts.values())
        new_shorts = {}
        for path, cand in shorts.items():
            if counts[cand] == 1:
                unique[path] = cand
            else:
                # Extend the candidate with the next directory up.
                rhs = path[:len(path)-len(cand)].strip(os.path.sep)
                new_shorts[path] = os.path.join(os.path.basename(rhs), cand)
        return extend(new_shorts, unique)
