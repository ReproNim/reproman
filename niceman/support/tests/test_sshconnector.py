...
from niceman.support.sshconnector import SSHConnection, SSHManager
from niceman.tests.utils import assert_raises, eq_
from niceman.tests.utils import skip_ssh, with_tempfile, get_most_obscure_supported_name
...
def test_ssh_manager_close():
    ...
    manager.get_connection('ssh://niceman-test').open()
    ok_(exists(opj(manager.socket_dir, 'localhost')))
    ok_(exists(opj(manager.socket_dir, 'niceman-test')))

    manager.close()

    ok_(not exists(opj(manager.socket_dir, 'localhost')))
    ok_(not exists(opj(manager.socket_dir, 'niceman-test')))
